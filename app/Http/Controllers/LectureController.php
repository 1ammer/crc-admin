...
//        return Storage::download($req,$nae);
    ...
    public function AssignmentMarksStore(Request $request) {
        ...
        try {
            ...
            $assgnmentmarks =(int) (($request->amarks / $request->atotal) * 10);
            
             

            $fResult = \App\FinalResult::
                    where('user_id', $request->userId)->where('course_id', $request->courseId)->first();
            if (!$fResult   ) {
                $fResult = new \App\FinalResult();
                $fResult->assign_marks = $assgnmentmarks;

            }else if($fResult->assign_marks==0){
                $fResult->assign_marks =$assgnmentmarks;
            } 
            else {
